...
假设有三个客户端要使用 sing-box，但它们的版本不同：

- iOS 客户端：版本是 sing-box 1.11（未更新到最新）
- 软路由（OpenWrt）：使用最新版 sing-box 1.12
- 安卓端：使用最新版 sing-box 1.12

因为 sing-box 从 1.12 开始对旧版本不兼容，三个客户端需要各自兼容的远端配置文件，但这些配置文件里要包含相同的订阅节点信息（即节点内容一致，只是配置格式不同）。

为此流程如下：

1. 为不同客户端准备不同的 sing-box 模板（例如 ios、linux、android）。
2. 使用 [Singbox Subscribe Convert](https://github.com/haierkeys/singbox-subscribe-convert) 服务，把相同的订阅节点信息套入不同模板，生成三个兼容各自版本的远端配置文件。
3. 最终会得到三个不同的配置访问地址，例如：
   - iOS: https://sb.xx.com/?password=123&template=ios
   - 软路由(OpenWrt): https://sb.xx.com/?password=123&template=linux
   - 安卓: https://sb.xx.com/?password=123&template=android

此外，可以用 Sub-Store 来集中管理所有 sing-box 节点，再用 [Singbox Subscribe Convert](https://github.com/haierkeys/singbox-subscribe-convert) 根据需要导出不同模板的远端配置。
